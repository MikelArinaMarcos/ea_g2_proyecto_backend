...
export class CommentController {
  ...
  public async createComment(req: Request, res: Response) {
    try {
      ...
      if (
        ...
        await this.user_service.addCommentToUser(
          req.body.users,
          comment_data._id
        );
        await this.activity_service.addCommentToActivity(
          req.body.activities,
          comment_data
        );
  ...
  public async getComment(req: Request, res: Response) {
    try {
      if (req.params.id) {
        ...
        const comment_data =
          await this.comment_service.filterComment(comment_filter);
        // Send success response
        return res
          .status(200)
          .json({ data: comment_data, message: 'Successful' });
      } else {
        return res.status(400).json({ error: 'Missing fields' });
      }
    } catch (error) {
  ...
  public async getComments(req: Request, res: Response) {
    try {
      if (req.params.page && req.params.id) {
        ...
        const comment_list = await this.comment_service.get5Comments(
          page_filter,
          id_filter
        );
  ...
  public async getLength(req: Request, res: Response) {
    try {
      ...
      const comment_length =
        await this.comment_service.commentLength(id_filter);
  ...
  public async updateComment(req: Request, res: Response) {
    try {
      if (req.params.id) {
        ...
        return res
          .status(200)
          .json({ data: new_comment_data, message: 'Successful' });
      } else {
  ...
  public async deleteComment(req: Request, res: Response) {
    try {
      if (req.params.id) {
        const delete_details = await this.comment_service.deleteComment(
          req.params.id
        );
