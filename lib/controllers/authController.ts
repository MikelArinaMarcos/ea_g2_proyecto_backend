...
export class AuthController {
    ...
    public async signin(req: Request, res: Response): Promise<Response> {
        ...
        return res.status(201).json({ token: token, refreshToken: refreshToken, id: userFound._id });
    };
    ...
    public async refreshToken(req: Request, res: Response): Promise<Response> {
        try {
            ...
            const decoded = jwt.verify(refreshToken, this._REFRESH_SECRET) as IJwtPayload;
            ...
            return res.status(201).json({ token: new_token, refreshToken: new_refreshToken });
        } catch (error) {
